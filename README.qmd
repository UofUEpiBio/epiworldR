...
**Note:** Measles-specific models have been moved to the separate [`measles`](https://github.com/UofUEpiBio/measles) R package.
...
Or from [R-universe](https://uofuepibio.r-universe.dev/){target="_blank"} (recommended for the latest development version):
...
```{r sir-plot}
#| fig-alt: "State trajectories for the SIR random-graph example over time."
plot(sir)
```

```{r sir-incidence}
#| fig-alt: "Incidence curve for the SIR random-graph example."
plot_incidence(sir)
```
...
```{r seir-conn-plot}
#| fig-alt: "State trajectories for the fully connected SEIR example over time."
plot(model_seirconn)
```

```{r seir-conn-repnum}
#| fig-alt: "Estimated reproductive number over time for the fully connected SEIR example."
...
```{r seir-conn-gentime}
#| fig-alt: "Generation time over time for the fully connected SEIR example."
...
```{r logit-model-plot}
#| fig-alt: "State trajectories for the SIR logit example over time."
plot(model_logit)
```

```{r logit-model-analysis}
...
```{r transmission-net}
#| fig-alt: "Transmission network from the simulated SIR outbreak, shown as a node-link graph."
# Creating a SIR model
sir <- ModelSIR(
  name           = "COVID-19",
  prevalence     = .01,
...
```{r multiple-example}
#| fig-alt: "Plot of reproductive-number results from multiple SIR simulations."
