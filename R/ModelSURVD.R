...
#' @export
#' @family Models
#' @aliases epiworld_survd
#' @returns
#' - The `ModelSURVD`function returns a model of class [epiworld_model].
#' @examples
#' model_survd <- ModelSURVD(
#'   name                  = "COVID-19",
#'   prevalence            = 20,
#'   efficacy_vax          = 0.6,
#'   latent_period         = 4,
...
#' run(model_survd, ndays = 100, seed = 1912)
#' model_survd 
#' 
#' # Plotting
#' plot(model_survd, main = "SURVD Model")
#' 
...
ModelSURVD <- function(
  ...
  structure(
    ModelSURVD_cpp(name, prevalence, efficacy_vax, latent_period, infect_period, 
    ...
    class = c("epiworld_survd", "epiworld_model")
  )
...
plot.epiworld_survd <- function(x, main = get_name(x), ...) { # col = NULL
